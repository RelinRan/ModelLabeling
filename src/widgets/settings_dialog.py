...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings: ProjectSettings, parent=None) -> None:
        ...
        self.setObjectName("settingsDialog"); self.resize(500, 380); self.setMinimumSize(500, 380); self.setWindowTitle("\u5e94\u7528\u8bbe\u7f6e")
        ...
        self.shape_combo = QComboBox(); self.shape_combo.addItem("\u77e9\u5f62", ShapeType.RECTANGLE); self.shape_combo.addItem("\u6b63\u65b9\u5f62", ShapeType.SQUARE); self.shape_combo.addItem("\u591a\u8fb9\u5f62", ShapeType.POLYGON); self.shape_combo.setCurrentIndex(max(0, self.shape_combo.findData(self.settings.enabled_shapes[0])))
        self.shape_combo.addItem("关键点" if not self.english else "Keypoints", ShapeType.KEYPOINT)
    ...
    def _page(self, title: str, rows: list[tuple[str, QWidget]]) -> QWidget:
        page = QWidget(); layout = QVBoxLayout(page); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(0); layout.addWidget(QLabel(title)); layout.addSpacing(ROW_SPACING); form = configure_form(QFormLayout())
    ...
    def _annotation_page(self) -> QWidget: return self._page("Display" if self.english else "\u6807\u6ce8\u663e\u793a", [("Shape" if self.english else "\u6807\u6ce8\u5f62\u72b6", self.shape_combo), ("Line width" if self.english else "\u7ebf\u6761\u7c97\u7ec6", self.line_width), ("Text size" if self.english else "\u6587\u5b57\u5927\u5c0f", self.text_size)])
    def _auto_page(self) -> QWidget: return self._page("Auto Label" if self.english else "\u81ea\u52a8\u6807\u6ce8", [("ONNX model", self._path_row(self.onnx_model, self._choose_model)), ("Input size" if self.english else "\u8f93\u5165\u5c3a\u5bf8", self.input_size_row), ("Confidence" if self.english else "\u7f6e\u4fe1\u5ea6\u9608\u503c", self.confidence), ("NMS threshold" if self.english else "NMS \u9608\u503c", self.nms), ("Save mode" if self.english else "\u6807\u6ce8\u4fdd\u5b58", self.auto_save)])
    def _general_page(self) -> QWidget: return self._page("General" if self.english else "\u901a\u7528\u8bbe\u7f6e", [("Application language" if self.english else "\u5e94\u7528\u8bed\u8a00", self.language_combo)])
    ...
    def _refresh_task_options(self) -> None:
        ...
        options = {
            "yolo": [("YOLO Detection" if self.english else "YOLO 检测", "yolo_detection"), ("YOLO Segmentation" if self.english else "YOLO 分割", "yolo_segmentation"), ("YOLO Pose" if self.english else "YOLO 关键点", "yolo_pose")],
    ...
    def _refresh_shape_options(self) -> None:
        ...
        for index in range(self.shape_combo.count()):
            shape = self.shape_combo.itemData(index)
            self.shape_combo.model().item(index).setEnabled(shape in supported)
        if current not in supported:
            first = next((index for index in range(self.shape_combo.count()) if self.shape_combo.itemData(index) in supported), 0)
            self.shape_combo.setCurrentIndex(first)
