...
from PySide6.QtWidgets import QApplication, QFileDialog, QDialog, QFrame, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMenu, QPlainTextEdit, QProgressBar, QSplitter, QStatusBar, QVBoxLayout, QWidget
...
class DatasetCountWorker(QObject):
    ...
    def __init__(self, image_dir: Path, session_id: str) -> None:
        super().__init__()
        self.image_dir, self.session_id = image_dir, session_id
...
class SingleImageAnnotationWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            annotation_dir = self.annotation_dir if self.settings.annotation_format == "coco" else self.annotation_dir / relative_parent
            if self.settings.annotation_format == "yolo":
                self.settings.label_presets = self._dataset_yolo_presets()
            result = AnnotationService().load(
                self.record.path, annotation_dir, self.settings, image_size=(width, height)
            )
...
class DatasetStatisticsWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            repository = DatasetIndexRepository(
                self.image_dir.parent.resolve(), self.image_dir, self.annotation_dir,
                self.settings.annotation_format,
            )
            records = self.records
            if records is None:
                # Dataset loading has already built this persistent index.
                # Reuse it instead of walking the image tree a second time;
            ...
            # YOLO and VOC resolve one annotation file from the image's
            # relative directory, so a global annotation index only adds a
            # long, invisible pause. COCO needs one JSON lookup table.
            index = service.build_index(
                self.annotation_dir,
                self.settings.annotation_format,
                cancel_callback=lambda: self.cancelled,
            ) if self.settings.annotation_format == "coco" else None
            ...
            with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="annotation-stats") as pool:
                for start in range(0, total, 256):
                    ...
                    futures = [pool.submit(self._read_labels, record, index) for record in batch]
                    for offset, future in enumerate(futures):
                        if self.cancelled:
                            return
                        record_labels = future.result()
                        if record_labels:
                            labeled += 1
                            total_labels += len(record_labels)
                            labels.update(record_labels)
                        current = start + offset + 1
                        snapshot = self._snapshot(total, labeled, total_labels, labels)
                        self.progress.emit(current, total, snapshot)
            snapshot = self._snapshot(total, labeled, total_labels, labels)
    ...
    @staticmethod
    def _snapshot(total: int, labeled: int, total_labels: int, labels: Counter) -> dict:
        return {
            "total_images": total,
    ...
    def _read_labels(self, record: ImageRecord, index) -> list[str]:
        """Read only label names for statistics; avoid building graphics objects."""
        if self.settings.annotation_format == "coco":
            relative_name = record.path.relative_to(self.image_dir).as_posix()
            image_item = index.coco_images.get(relative_name) or index.coco_images.get(record.path.name)
            if not image_item:
                return []
            categories = index.coco_categories
            return [
                str(categories.get(item.get("category_id"), f"class_{item.get('category_id')}"))
                for item in index.coco_annotations.get(image_item.get("id"), [])
                if item.get("bbox") and len(item.get("bbox", [])) >= 4
            ]

        relative_parent = record.path.parent.relative_to(self.image_dir)
        target_dir = self.annotation_dir / relative_parent
        suffix = ".xml" if self.settings.annotation_format == "voc" else ".txt"
        annotation_path = target_dir / f"{record.path.stem}{suffix}"
        if not annotation_path.exists():
            annotation_path = self.annotation_dir / f"{record.path.stem}{suffix}"
        if not annotation_path.exists():
            return []
        try:
            if self.settings.annotation_format == "voc":
                ...
                return [
                    name
                    for name in (item.findtext("name", "").strip() for item in root.findall("object"))
                    if name
                ]
            result = []
            for line in annotation_path.read_text(encoding="utf-8").splitlines():
                ...
                try:
                    ...
                    continue
                result.append(self._class_names.get(class_id, f"class_{class_id}"))
            return result
...
class DatasetScanWorker(QObject):
    ...
    def __init__(self, image_dir: Path, annotation_dir: Path, settings: ProjectSettings, dataset_root: Path | None = None, session_id: str = "") -> None:
        super().__init__()
        self.image_dir, self.annotation_dir, self.settings = image_dir, annotation_dir, settings
        self.dataset_root = Path(dataset_root or image_dir.parent).resolve()
    ...
    def run(self) -> None:
        try:
            # Index paths first. Pixel metadata and annotations are loaded only
            # when an image is selected, so a large dataset becomes usable
            # after the first index batch instead of after a full parse.
            repository = DatasetIndexRepository(
                self.dataset_root, self.image_dir, self.annotation_dir,
                self.settings.annotation_format,
            )
            parser_presets = self._discover_presets(sample_limit=100)
            if repository.is_complete():
                self.partial.emit(DatasetScanResult([], parser_presets, repository.count(), True, self.session_id))
            repository.set_complete(False)
            indexed = 0
            first_batch_emitted = False
            for batch in repository.scan_paths(
                lambda: self.cancelled,
                batch_size=500,
                label_names=[preset.name for preset in parser_presets],
            ):
                repository.upsert_batch(batch)
                indexed += len(batch)
                if not first_batch_emitted:
                    records = [self._record_from_index(item) for item in batch]
                    self.partial.emit(DatasetScanResult(records, parser_presets, 0, True, self.session_id))
                    first_batch_emitted = True
                self.progress.emit(indexed, self.total_count)
            if self.cancelled:
                return
            repository.prune_missing(lambda: self.cancelled)
            total = repository.count()
            repository.set_complete(True)
            self.progress.emit(total, total)
            self.finished.emit(DatasetScanResult([], parser_presets, total, True, self.session_id))
            return
        except Exception as exc:
            self.failed.emit(str(exc))
    ...
    def _cache_path(self) -> Path:
        cache_dir = Path.home() / "AppData" / "Local" / "ModelLabeling" / "cache"
        cache_dir.mkdir(parents=True, exist_ok=True)
        key = hashlib.sha1(str(self.dataset_root).casefold().encode("utf-8")).hexdigest()
        return cache_dir / f"dataset_{key}.json"

    def _load_cache_records(self) -> list[ImageRecord]:
        """Load the complete cached index only after the preview is visible."""
        path = self._sqlite_cache_path()
        with sqlite3.connect(path) as connection:
            return [ImageRecord(
                Path(row[0]), int(row[1]), int(row[2]), row[3], int(row[4]),
                [], "pending", row[6], False,
            ) for row in connection.execute(
                "SELECT path, width, height, file_format, file_size, status, error, annotations FROM images ORDER BY sort_key"
            )]

    def _sqlite_cache_path(self) -> Path:
        return self._cache_path().with_suffix(".sqlite3")

    def _directory_signature(self) -> str:
        entries = []
        for base in (self.image_dir, self.annotation_dir):
            if not base.exists():
                continue
            for root, dirs, _files in os.walk(base):
                ...
                try:
                    ...
                    continue
                entries.append((str(Path(root).resolve()), stat.st_mtime_ns, len(dirs)))
        return hashlib.sha1(json.dumps(entries, separators=(",", ":")).encode("utf-8")).hexdigest()

    def _manifest(self, cancel_callback=None) -> list[dict]:
        paths = set()
        ...
        for path in sorted(paths):
            try:
                stat = path.stat()
                manifest.append({"path": str(path.resolve()), "size": stat.st_size, "mtime_ns": stat.st_mtime_ns})
            except OSError:
                return []
        return manifest

    def _load_cache(self) -> tuple[list[ImageRecord], list[LabelPreset]] | None:
        cached = self._load_sqlite_cache()
        if cached is not None:
            return cached
        path = self._cache_path()
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            if payload.get("format") != self.settings.annotation_format or payload.get("manifest") != self._manifest():
                return None
            records = []
            for item in payload.get("records", []):
                records.append(ImageRecord(
                    Path(item["path"]), int(item["width"]), int(item["height"]), item["file_format"], int(item["file_size"]),
                    [Annotation.from_dict(value) for value in item.get("annotations", [])], item.get("status", "unlabeled"), item.get("error"),
                ))
            presets = [LabelPreset.from_dict(value) for value in payload.get("presets", [])]
            return records, presets
        except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError):
            return None

    def _load_cache_fast(self) -> tuple[list[ImageRecord], list[LabelPreset], str] | None:
        path = self._sqlite_cache_path()
        if not path.exists():
            legacy = self._load_cache()
            return (*legacy, "") if legacy is not None else None
        try:
            with sqlite3.connect(path) as connection:
                meta = dict(connection.execute("SELECT key, value FROM metadata"))
                if meta.get("format") != self.settings.annotation_format:
                    return None
                # Do not walk a large dataset before showing cached rows. The
                # caller validates the full manifest after the cached preview
                # is already visible and rescans if the cache is stale.
                presets = [LabelPreset.from_dict(item) for item in json.loads(meta.get("presets", "[]"))]
                records = [ImageRecord(
                    Path(row[0]), int(row[1]), int(row[2]), row[3], int(row[4]),
                    [], "pending", row[6], False,
                ) for row in connection.execute(
                    "SELECT path, width, height, file_format, file_size, status, error, annotations FROM images ORDER BY sort_key LIMIT 100"
                )]
                return records, presets, meta.get("manifest", "")
        except (OSError, sqlite3.Error, json.JSONDecodeError, KeyError, TypeError, ValueError):
            return None

    def _load_sqlite_cache(self) -> tuple[list[ImageRecord], list[LabelPreset]] | None:
        path = self._sqlite_cache_path()
        if not path.exists():
            return None
        try:
            manifest = json.dumps(self._manifest(), ensure_ascii=False, separators=(",", ":"))
            with sqlite3.connect(path) as connection:
                meta = dict(connection.execute("SELECT key, value FROM metadata"))
                if meta.get("format") != self.settings.annotation_format or meta.get("manifest") != manifest:
                    return None
                presets = [LabelPreset.from_dict(item) for item in json.loads(meta.get("presets", "[]"))]
                records = []
                for row in connection.execute(
    ...
    def _save_cache(self, records: list[ImageRecord], presets: list[LabelPreset]) -> None:
        self._save_sqlite_cache(records, presets)

    def _save_sqlite_cache(self, records: list[ImageRecord], presets: list[LabelPreset]) -> None:
        path = self._sqlite_cache_path()
        temp = path.with_suffix(".tmp.sqlite3")
        manifest = json.dumps(self._manifest(), ensure_ascii=False, separators=(",", ":"))
        directory_signature = self._directory_signature()
        try:
            ...
            with sqlite3.connect(temp) as connection:
                ...
                connection.execute("CREATE TABLE images (path TEXT PRIMARY KEY, sort_key TEXT NOT NULL, width INTEGER NOT NULL, height INTEGER NOT NULL, file_format TEXT NOT NULL, file_size INTEGER NOT NULL, status TEXT NOT NULL, error TEXT, annotations TEXT NOT NULL)")
                connection.executemany("INSERT INTO metadata(key, value) VALUES (?, ?)", [
                    ("format", self.settings.annotation_format),
                    ("manifest", manifest),
                    ("directory_signature", directory_signature),
                    ("presets", json.dumps([preset.to_dict() for preset in presets], ensure_ascii=False, separators=(",", ":"))),
                ])
                connection.executemany(
                    "INSERT INTO images(path, sort_key, width, height, file_format, file_size, status, error, annotations) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [(
                        str(record.path), record.path.name.casefold(), record.width, record.height,
                        record.file_format, record.file_size, record.status, record.error,
                        json.dumps([annotation.to_dict() for annotation in record.annotations], ensure_ascii=False, separators=(",", ":")),
    ...
    def _save_json_cache_legacy(self, records: list[ImageRecord], presets: list[LabelPreset]) -> None:
        payload = {
            "format": self.settings.annotation_format,
            "manifest": self._manifest(),
            "presets": [preset.to_dict() for preset in presets],
            "records": [
                {"path": str(record.path), "width": record.width, "height": record.height, "file_format": record.file_format,
                 "file_size": record.file_size, "status": record.status, "error": record.error,
                 "annotations": [annotation.to_dict() for annotation in record.annotations]}
                for record in records
            ],
        }
        path = self._cache_path()
    ...
    def _discover_presets(self, sample_limit: int | None = None) -> list[LabelPreset]:
        names: list[str] = []
        root = self.image_dir.parent
        if self.settings.annotation_format == "yolo":
            for path in (root / "classes.txt", root.parent / "classes.txt"):
                if path.exists():
                    ...
                    break
            if not names:
                for path in (root / "data.yaml", root / "data.yml", root.parent / "data.yaml", root.parent / "data.yml"):
                    if path.exists():
                        content = path.read_text(encoding="utf-8")
                        match = re.search(r"names:\s*\{([^}]*)\}", content, re.IGNORECASE)
                        if match:
                            pairs = re.findall(r"(?:\d+)\s*:\s*['\"]?([^,'\"}]+)", match.group(1))
                            names = [item.strip() for item in pairs]
                        break
            ...
            for index, path in enumerate(self.annotation_dir.rglob("*.xml")):
                ...
                try:
                    ...
                    names.extend((node.findtext("name") or "").strip() for node in root_node.findall("object"))
            ...
            for path in sorted(self.annotation_dir.glob("*.json")):
                try:
                    ...
                    names.extend(str(item.get("name", "")).strip() for item in document.get("categories", []))
                    break
...
class FileTextDialog(QDialog):
    def __init__(self, title: str, content: str, parent=None) -> None:
        ...
        editor.customContextMenuRequested.connect(
            lambda position, field=editor: self._show_text_menu(field, position)
        )
    ...
    def _resize_to_content(self, editor: QPlainTextEdit, content: str) -> None:
        ...
        width = max(320, min(max_width, longest_line + frame_width))
        height = max(120, min(max_height, len(lines) * metrics.lineSpacing() + frame_height))
...
class FileInfoDialog(QDialog):
    def __init__(self, title: str, rows: list[tuple[str, str]], parent=None) -> None:
        ...
        for row, (label, value) in enumerate(rows):
            ...
            val.setFrame(False)
            val.setClearButtonEnabled(False)
