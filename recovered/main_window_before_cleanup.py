...
class MainWindow(QMainWindow):
    ...
    def _dataset_scan_finished(self, records):
        ...
        if worker is not None and worker.cancelled:
            ...
            return
        self.state.images = records
        self.state.current_index = 0 if records else -1
        self.refresh_image_list()
        self._set_dataset_loading(False)
        self._close_task_list()
        if self.isVisible():
            AppDialog.information("打开数据集", "数据集加载成功", self)
    ...
    def _dataset_scan_failed(self, message):
        ...
        self._close_task_list()
        if self.isVisible():
            AppDialog.information("打开数据集失败", message, self)
